...
Regcont = pd.read_csv(DataFile + 'fig2_output_all_calculated_data.dat')   
...
Data_high2 = np.array(h5py.File(DataFile + 'Data_ZHigh_fuhe_test.mat')['Data_ZHigh']).T
Data_low2 = np.array(h5py.File(DataFile + 'Data_ZLow_fuhe_test.mat')['Data_ZLow']).T
Cond_downheight2 = np.array(h5py.File(DataFile + 'Cond_Downheight_fuhe_test.mat')['Cond_Downheight']).T   
...
plt.show()
plt.savefig(ResAnalyFile + 'Figure17_Testsamples.png', dpi=300)
plt.close()

# (2)Prediction test samples 
Regcont = pd.read_csv(DataFile + 'fig3_output_all_calculated_data.dat')   
Regcont=np.array(Regcont).T
x_original = np.linspace(0, Regcont.shape[1] - 1, Regcont.shape[1])
x_new = np.linspace(0, Regcont.shape[1] - 1, 501)
Regcont_interpolated = np.zeros((Regcont.shape[0], 501))
...
plt.show()
plt.savefig(ResAnalyFile + '/SampleData_testpredict.png', dpi=300)
