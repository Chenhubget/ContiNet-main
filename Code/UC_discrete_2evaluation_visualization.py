...
DataFile = parent_directory +'/Data/DataSet_UC/UTrainSet4/'
ResAnalyFile = parent_directory + '/Output/Results_UC/'    
...
for i in range(6):
    if i < 4:
        with open(ResAnalyFile + 'UTrainSet'+ str(i+1)+'/'+ 'Upkernel_pred_9.json','r') as f:
            Upkernel_pred = json.load(f)
    if i==4:
        with open(ResAnalyFile + 'UTrainSet'+ str(i+1)+'_Noise1/'+ 'Upkernel_pred_9.json','r') as f:
            Upkernel_pred = json.load(f)
    if i==5:
        with open(ResAnalyFile + 'UTrainSet'+ str(i)+'_Noise2/'+ 'Upkernel_pred_9.json','r') as f:
            Upkernel_pred = json.load(f)    
