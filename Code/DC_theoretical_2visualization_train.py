...
## Visualization 
# (1)Loss, R2 
File1 = 'run-20250419-160421No_Add_Normed_lr1ef4_epoch500_batch501_layer9_15_3_train-tag-epoch_'
File2 = 'run-20250419-125844With_Add_Normed_lr1ef4_epoch500_batch501_layer9_15_3_train-tag-epoch_'
...
plt.show()
plt.savefig(ResAnalyFile+'Figure12_Comparison_different_DnContiNet_structures.png', dpi=300, bbox_inches='tight')
plt.close()
...
for i in range(3):
    ...
    weight = linreg[0]
    model = load_model(ResAnalyFile + 'best_model_No_Add_Normed_lr1ef4_epoch500_batch501_layer9_15_3.h5',custom_objects={'R2':R2})
    ...
    model = load_model(ResAnalyFile + 'best_model_With_Add_Normed_lr1ef4_epoch500_batch501_layer9_15_3.h5',custom_objects={'R2':R2})
    ...
    Upkernel_inv = Upkernel_inv[:,int(501/2):251];  Kernel_pred2=Kernel_pred2[:,int(501/2):251]; Kernel_pred3single=Kernel_pred3[:,int(501/2):251]
    Data_temp = np.concatenate((Upkernel_inv,Kernel_pred2,Kernel_pred3single),axis=1).T
    ...
    for j in range(3):
       ...
       if j*3+i+1 in [3,6,9,12,15]:
            ...
            ax2.set_yticks([])   
plt.show()
plt.savefig(ResAnalyFile + '/Figure13_Weight_different_methods.png', dpi=300)
plt.close()
...
plt.show()
plt.savefig(ResAnalyFile + '/Figure14_Kernelmatrix_DnContiNet.png', dpi=300)
plt.close()
...
for i in range(3):
    ...
    if i == 0:
       plt.plot(np.linspace(-250, 250, 501),Data_high1[Index,:],c=Colors[i],label= 'RealFwd_30', linewidth=3,zorder=2)
    plt.plot(np.linspace(-250, 250, 501),Data_low1[Index,:],c=Colors[i+1],label= 'RealFwd_'+ str(int(30-Cond_downheight1[Index][0])), linewidth=3,zorder=2,linestyle='--')
    ...
    if i == 0: 
       plt.plot(np.linspace(-250, 250, 501),Data_high2[Index,:],c=Colors[i],label= 'RealFwd_30', linewidth=3,zorder=2)
    plt.plot(np.linspace(-250, 250, 501),Data_low2[Index,:],c=Colors[i+1],label= 'RealFwd_'+ str(int(30-Cond_downheight2[Index][0])), linewidth=3,zorder=2,linestyle='--')
    plt.xlabel('Points_obs',font={'family':'Times New Roman'},fontsize=25)
    plt.ylabel('Anomaly(nT)',font={'family':'Times New Roman'},fontsize=25)
    plt.xticks(np.linspace(-250, 250, 6),font={'family':'Times New Roman'},fontsize=22)
    plt.yticks(font={'family':'Times New Roman'},fontsize=22)
...
plt.show()
plt.savefig(ResAnalyFile + 'Figure15_Training_samples.png', dpi=300)
plt.close()

# (5)Training results
Colors = ['k','#d62728', '#ff7f0e','#2ca02c', '#17becf','#1f77b4', '#9467bd']  
model_N = load_model(ResAnalyFile + '/best_model_'+ 'No_Add_Normed_lr1ef4_epoch500_batch501_layer9_15_3' +'.h5',custom_objects={'R2':R2})
model_W = load_model(ResAnalyFile + '/best_model_'+ 'With_Add_Normed_lr1ef4_epoch500_batch501_layer9_15_3' +'.h5',custom_objects={'R2':R2})
...
for i in range(6):  
    ...
    if Sbplt[i] in [1,3,5]: Dim = 0
    else: Dim = 1
    ax.plot(np.linspace(-250, 250, 501),Data_low[Index,:][Dim,:],c='#1f77b4',linestyle='-',label= 'RealFwd_' +str(int(30-Cond_downheight[Index,:][Dim][0])), linewidth=1.5,zorder=2)
    ax.plot(np.linspace(-250, 250, 501),Out_N[Dim,:],c='#ff7f0e',linestyle='-',marker='o',markersize=0,label='UpContiNet_'+ str(int(30-Cond_downheight[Index,:][Dim][0])), linewidth=1.5,zorder=3)
    ax.plot(np.linspace(-250, 250, 501),Data_lowpred[Dim,:],c='#9467bd',linestyle='-',marker='*',markersize=0,label='DnContiNet_'+ str(int(30-Cond_downheight[Index,:][Dim][0])), linewidth=1.5,zorder=4)
    ax.plot(np.linspace(-250, 250, 501),Out_W[Dim,:],c='#2ca02c',linestyle='-',marker='^',markersize=0,label='DnMainNet_'+ str(int(30-Cond_downheight[Index,:][Dim][0])), linewidth=1.5,zorder=3)
    ax.plot(np.linspace(-250, 250, 501),label_embedding[Index,:][Dim] * Out_Add[Dim,:],linestyle='-.', c='#d62728',label='DnRegNet_'+ str(int(30-Cond_downheight[Index,:][Dim][0])),linewidth=1.5,zorder=1)
...
plt.show()
plt.savefig(ResAnalyFile + '/Figure16_Comparison_DCtraining_results..png', dpi=300)
